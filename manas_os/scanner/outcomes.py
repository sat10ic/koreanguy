    ...
    for c in rows:
        ...
        for horizon in HORIZONS:
            ...
            conn.execute(
                ...
                "forward_return_pct, forward_r, status, updated_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now')) "
                "ON CONFLICT(candidate_date, symbol, setup, horizon) DO UPDATE SET "
                "as_of_date=excluded.as_of_date, "
                "forward_return_pct=excluded.forward_return_pct, "
                "forward_r=excluded.forward_r, status=excluded.status, "
                "updated_at=datetime('now')",
                (c["candidate_date"], c["symbol"], c["setup"], horizon, as_of_date, fwd_pct, fwd_r, status),
            )
            written += 1
    return written
