# setup-selection-8 — GOLD v2

| field | value |
|-------|--------|
| source_file | Setup Selection 8.jpg |
| symbol | CHENNAI PETROLEUM CORP LTD · 1W+1D |
| family | `titan_multi_tf_coil` |
| status | **gold** |
| nuance | Setup Selection #8 TITAN selection-state. weekly reclaim after long decline |

JSON: `entries/json/setup-selection-8.json`

## pre_breakout_crop
- **crop_x:** setup-state right edge
- **decision_moment:** Right-edge daily tight+dry; weekly near 10w
...
- TITAN checklist on CHENNAI PETROLEUM CORP LTD
- Selection-state (no LF required)

## volume_signature
```
footprints mid → dry at right pivot
```

## unlabeled_detection_spec
**Family:** `titan_multi_tf_coil`
**Must-see:** TITAN pure geometry; ignore stickers at inference
