// Talks directly to the FastAPI backend on :8000 (CORS allows :5173/:5174; the
// new terminal runs on :5175 which gets added to the allow-list in api/app.py).
// Single-user local-first tool, no proxy, no env var.
...
/** GET /api/regime/breadth-analytics — 60-session quadrant history rows
 * (the MarketQuadrant pattern: each row renders its own history). */
export function getBreadthAnalytics(days = 60, date) {
  const params = new URLSearchParams();
  if (days != null) params.set("days", days);
  if (date) params.set("date", date);
  const qs = params.toString();
  return getJSON(`/api/regime/breadth-analytics${qs ? `?${qs}` : ""}`);
}

/** GET /api/regime/mswing — MOMENTUM row series + per-index table. */
export function getMswing(days = 90, date) {
  const params = new URLSearchParams();
  if (days != null) params.set("days", days);
  if (date) params.set("date", date);
  const qs = params.toString();
  return getJSON(`/api/regime/mswing${qs ? `?${qs}` : ""}`);
}
