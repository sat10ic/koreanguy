...
def upsert_features(conn, df):
    if df.empty:
        return
    cols = [
        'symbol', 'date', 'sma10', 'sma20', 'sma21', 'sma40', 'sma50', 'sma200',
    ...
    # Make sure every expected column exists in df, else fill with None
    for c in cols:
    ...
    cur.executemany(sql, out.values.tolist())
    conn.commit()


def run_indicators(progress_cb=None, force_rebuild: bool = False) -> dict:
    """Recompute features. By default only fills rows newer than the latest
    persisted date per symbol. Pass `force_rebuild=True` to overwrite the
    full history (used after adding new indicator columns).
    """
    _db.init_schemas()
...
def main():
    ...
    p.add_argument('--force-rebuild', action='store_true',
                   help='Overwrite full history (use after adding columns)')
    args = p.parse_args()
    res = run_indicators(
        progress_cb=lambda i, t, s, st: print(f'[{i}/{t}] {s} {st}'),
